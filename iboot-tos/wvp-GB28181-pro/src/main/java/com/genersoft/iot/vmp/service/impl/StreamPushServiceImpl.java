...
@Service
public class StreamPushServiceImpl extends ServiceImpl<StreamPushMapper, StreamPushItem> implements IStreamPushService {
    ...
    @Override
    public Page<StreamPushItem> getPushList(Page page, String query, Boolean pushing, String mediaServerId) {
        return streamPushMapper.selectAllForList(page, query, pushing, mediaServerId);
    }
    ...
    public boolean saveToGB(GbStream stream) {
        ...
        int add = gbStreamMapper.insert(stream);
        return add > 0;
    }
    ...
    public boolean removeFromGB(GbStream stream) {
        ...
        int del = gbStreamService.del(stream.getApp(), stream.getStream());
        ...
        if (mediaList != null) {
            if (mediaList.getInteger("code") == 0) {
                ...
                if (data == null) {
                    this.deleteByAppAndStream(stream.getApp(), stream.getStream());
                }
            }
        }
        return del > 0;
    ...
    public boolean stop(String app, String streamId) {
        ...
        gbStreamService.del(app, streamId);

        if (this.deleteByAppAndStream(app, streamId)) {
    ...
    public void zlmServerOffline(String mediaServerId) {
        ...
        gbStreamService.deleteWithoutGBId("push", mediaServerId);
        // 其他的流设置未启用
    ...
    public boolean add(StreamPushItem stream) {
        ...
        try {
            int addStreamResult = streamPushMapper.insert(stream);
            if (!ObjectUtils.isEmpty(stream.getGbId())) {
                stream.setStreamType("push");
                gbStreamMapper.insert(stream);
            }
    ...
    @Override
    public boolean deleteByAppAndStream(String app, String stream) {
        return streamPushMapper.delete(Wrappers.<StreamPushItem>lambdaQuery()
                .eq(StreamPushItem::getApp, app)
                .eq(StreamPushItem::getStream, stream)) > 0;
    }
}
